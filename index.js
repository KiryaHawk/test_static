...
function init() {
    fetch('VIP.json')
        ...
        .then(obj => {
            ...
            // Кнопка "Доли" (правый нижний угол)
            ...
            if (shareToggle) {
                shareToggle.addEventListener('click', () => {
                    ...
                    // визуально подсвечиваем активное состояние +
                    // включаем/выключаем отображение .shares-metrics
                    if (showRegionsOnly) {
...
function setupFilterUI(minQuantity, maxQuantity, objectManager) {
    ...
    // элементы фильтра по соотношению
    ...
    // панель по ДК изначально скрыта
    ...
    // открыть/закрыть панель категорий
    ...
    if (ratioResetBtn) {
        ratioResetBtn.addEventListener('click', () => {
            ratioFilterMode = 'all';
            if (ratioRadios && ratioRadios.length) {
                ratioRadios.forEach(r => r.checked = false);
            }
            applyFilter(currentMinQuantity, objectManager);
        });
    }
...
function applyFilter(minQuantity, objectManager) {
    ...
    objectManager.setFilter(obj => {
        ...
        // 1. ГИБДД (синие точки) — зависят ТОЛЬКО от флага showGibdd,
        // режим "Доли" и категории на них не влияют
        if (isBlue) {
            return showGibdd;
        }

        // 2. Обычные точки: фильтр по региону ("Доли")
        ...
        if (ratioFilterMode !== 'all') {
            ...
            if (ratioFilterMode === 'growth') {
                // Рост: только строго положительные значения
                if (!(r !== null && r > 0 && !isZero)) {
                    return false;
                }
            } else if (ratioFilterMode === 'fall') {
                // Падение: только строго отрицательные значения
                if (!(r !== null && r < 0 && !isZero)) {
                    return false;
                }
            } else if (ratioFilterMode === 'none') {
                // Отсутствие: пустые или 0
                if (!(r === null || isZero)) {
                    return false;
                }
            }

...
let showRegionsOnly = false; // флаг режима "Доли" (только точки с регионом)
let selectedCategories = new Set(); // выбранные категории для фильтра
let ratioFilterMode = 'all'; // 'all' | 'growth' | 'fall' | 'none'
            ...
            if (shareToggle) {
                shareToggle.addEventListener('click', () => {
                    ...
                    if (showRegionsOnly) {
                        ...
                        body.classList.add('shares-on');
                    } else {
                        shareToggle.classList.remove('active');
                        body.classList.remove('shares-on');

                        // при выключении "Доли" сбрасываем фильтр по соотношению
                        ratioFilterMode = 'all';
                        const ratioRadios = document.querySelectorAll('input[name="ratio-mode"]');
                        Array.from(ratioRadios).forEach(r => r.checked = false);
                    ...
                    updateShareHighlightColors(objectManager); // перекраска подсвеченных точек
                });
            }

            // Фильтры по ДК / категориям / ГИБДД / соотношению
...
function setupFilterUI(minQuantity, maxQuantity, objectManager) {
    ...
    const ratioPanel = document.getElementById('ratio-panel');
    const ratioRadios = document.querySelectorAll('input[name="ratio-mode"]');
    const ratioResetBtn = document.getElementById('ratio-reset');
    ...
    gibddToggle.addEventListener('click', () => {
        ...
        applyFilter(currentMinQuantity, objectManager);
        updateShareHighlightColors(objectManager);
    });
    ...
    // === ФИЛЬТР ПО СООТНОШЕНИЮ ===
    if (ratioToggle && ratioPanel) {
        ratioPanel.style.display = 'none';
        ratioToggle.addEventListener('click', () => {
            const visibleNow = ratioPanel.style.display === 'block';
            ratioPanel.style.display = visibleNow ? 'none' : 'block';
            console.log('ratio panel now:', ratioPanel.style.display);
        });
    }

    if (ratioRadios && ratioRadios.length) {
        Array.from(ratioRadios).forEach(radio => {
            radio.addEventListener('change', () => {
                if (!radio.checked) return;
                const v = radio.value;
                if (v === 'growth' || v === 'fall' || v === 'none') {
                    ratioFilterMode = v;
                } else {
                    ratioFilterMode = 'all';
                }
...
function applyFilter(minQuantity, objectManager) {
    ...
    objectManager.setFilter(obj => {
        ...
        // ГИБДД — только по флагу showGibdd
        if (isBlue) {
            return showGibdd;
        }

        // Фильтр по региону ("Доли")
        const region = typeof props.region === 'string'
            ? props.region.trim()
            : '';

        if (showRegionsOnly && !region) {
            return false;
        }
        ...
        // Фильтр по соотношению
        if (ratioFilterMode !== 'all') {
            let r = null;

            if (typeof props.ratio === 'number') {
                r = props.ratio;
            } else if (typeof props.ratio === 'string' && props.ratio.trim() !== '') {
                const parsed = parseFloat(props.ratio.replace(',', '.'));
                if (!isNaN(parsed)) r = parsed;
            }

            const isZero = (r !== null) ? Math.abs(r) < 1e-9 : false;

            if (ratioFilterMode === 'growth') {
                if (!(r !== null && r > 0 && !isZero)) {
                    return false;
                }
            } else if (ratioFilterMode === 'fall') {
                if (!(r !== null && r < 0 && !isZero)) {
                    return false;
                }
            } else if (ratioFilterMode === 'none') {
                if (!(r === null || isZero)) {
                    return false;
                }
            }
...
/**
 * Перекраска точек, залитых в Excel (isHighlighted),
 * когда включён режим "Доли".
 * До нажатия "Доли" все точки зелёные/красные/синие по пресетам.
 */
function updateShareHighlightColors(objectManager) {
    if (!objectManager) return;

    const shareOn = showRegionsOnly;
    ...
    objectManager.objects.each(function (obj) {
        ...
        // базовый цвет по пресету
        let baseColor = null;
        if (preset === 'islands#greenIcon') {
            baseColor = '#3caa3c';
        } else if (preset === 'islands#redIcon') {
            baseColor = '#ed4543';
        } else if (preset === 'islands#blueIcon') {
            baseColor = '#1e98ff';
        }

        if (shareOn && isHighlighted && !isBlue) {
            // включены "Доли" и точка подсвечена в Excel → делаем оранжевой
            objectManager.objects.setObjectOptions(objectId, {
                preset: preset,
                iconColor: '#ff9900'
            });
        } else {
            // обычный режим — возвращаем базовый цвет
            if (baseColor) {
                objectManager.objects.setObjectOptions(objectId, {
                    preset: preset,
                    iconColor: baseColor

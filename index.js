...
function init() {
    fetch('VIP.json')
        ...
        .then(obj => {
            ...
            // Кнопка "Доли" (показывать только точки с заполненным регионом)
            const shareButton = new ymaps.control.Button({
                data: {
                    content: 'Доли',
                    title: 'Показать только точки с указанным регионом'
                },
                options: {
                    selectOnClick: true, // кнопка-переключатель
                    maxWidth: [80, 100, 120]
                }
            });

            myMap.controls.add(shareButton, {
                float: 'none',
                position: {
                    bottom: 30,
                    right: 20
            ...
            // Включение режима "Доли"
            shareButton.events.add('select', () => {
                showRegionsOnly = true;
                applyFilter(currentMinQuantity, objectManager);
            });

            // Выключение режима "Доли"
            shareButton.events.add('deselect', () => {
                showRegionsOnly = false;
                applyFilter(currentMinQuantity, objectManager);
            });
...
function applyFilter(minQuantity, objectManager) {
    ...
    objectManager.setFilter(obj => {
        ...
        // Регион: берём либо "region", либо "Регион"
        const region =
            (props.region !== undefined && props.region !== null ? props.region : '') ||
            (props['Регион'] !== undefined && props['Регион'] !== null ? props['Регион'] : '');

        // Режим "Доли": показываем только точки, у которых регион не пустой
        if (showRegionsOnly && (!region || String(region).trim() === '')) {
            return false;
        }
